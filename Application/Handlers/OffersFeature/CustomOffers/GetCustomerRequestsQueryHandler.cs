...
{
    internal class GetCustomerRequestsQueryHandler : IRequestHandler<GetCustomerRequestsQuery, Result<List<CustomerOfferResponse>>>
    {
        private readonly D2DContext _context;

        public GetCustomerRequestsQueryHandler(D2DContext context)
        {
            _context = context;
        }

        public async Task<Result<List<CustomerOfferResponse>>> Handle(GetCustomerRequestsQuery request, CancellationToken cancellationToken)
        {
            var design = _context.CustomerCustomOffers
                ...
                .Where(d => d.CustomerID == request.CustomerId && d.CustomerOfferStatus == OfferStatus.OnHold && !d.IsActive && !d.IsDeleted);
